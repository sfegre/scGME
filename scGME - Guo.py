...
def _scclust(X, X_raw, sf, dip_merge_threshold, batch_size, learning_rate, pretrain_epochs, embedding_size, dedc_epochs,lambda_1, lambda_2, n_clusters_max, n_clusters_min,n,optimizer_class=torch.optim.Adam, loss_fn=torch.nn.MSELoss()):
    ...
    print("embedded_data",embedded_data.shape,embedded_data)

    # Execute Louvain algorithm to get initial micro-clusters in embedded space
    imputer = SimpleImputer(strategy='mean')
    embedded_data = imputer.fit_transform(embedded_data)
    ...
    cluster_labels_cpu, n_clusters_current, centers_cpu, autoencoder = _scclust_training(X, X_raw, sf, n_clusters_start,
                                                                                          ...
                                                                                          trainloader,
                                                                                          testloader)
    #(X, n_clusters_current, dip_merge_threshold, centers_cpu,piper, debug):

    return cluster_labels_cpu, n_clusters_current, centers_cpu, autoencoder

class SCClust():
    def __init__(self, dip_merge_threshold=0.9, batch_size=256,learning_rate=0.001, pretrain_epochs=50, embedding_size=47, dedc_epochs=10, lambda_1=2.5, lambda_2=1.5, n_clusters_max=np.inf, n_clusters_min=5,n=90):
        #dip_merge_threshold=0.9, batch_size=128,learning_rate=1e-4, pretrain_epochs=100, embedding_size=10,dedc_epochs=50, lambda_1=1.0, lambda_2=0.5, n_clusters_max=np.inf, n_clusters_min=3
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', type=str, default='Guo')
    ...
    #data_mat = h5py.File(args.data_path + "/" + args.name + ".h5")
    #x1 = np.array(data_mat['X1'])
    #x2 = np.array(data_mat['X2'])
    #Y = np.array(data_mat['Y'])
    #data_mat.close()

    data = pd.read_csv(args.data_path + "/" + args.name + "/" +"data.csv", index_col=0, header=0, sep=',', encoding='utf-8', engine='c')
    y = pd.read_csv(args.data_path + "/" + args.name + "/" +"label.csv", encoding='utf-8')
    #data, y = pro.read_data(filename=args.data_path + "/" + args.name, data_type=args.data_type)
    ...
    top_indices = sorted_indices[:15000]
    ...
    #adata_triku = adata[:, adata.var["highly_variable"] == True]

    #highly_gene = adata.var["highly_variable"]
    #print(highly_gene)
    #X_triku = adata.X[:, highly_gene]
    #print("adata_triku.X", adata_triku.X.shape, adata_triku.X)
    ...
    # louvain
    #sc.pp.neighbors(adata, n_neighbors=15, use_rep="X")  # 计算邻域图
    #adata0 = sc.tl.louvain(adata, resolution=3, random_state=0, copy=True)
    # 分辨率越高就会找到更多更小的集群
    # sc.tl.louvain实际上为adata0添加obs，obs的名称是louvain，即一个简单的分类
    #y_pred = adata0.obs['louvain']
    #cluster_labels = np.asarray(y_pred, dtype=int)


    #adata_triku.raw=adata_triku.copy()
    #sc.pp.normalize_per_cell(adata_triku)
    #adata_triku.obs['size_factors'] = adata_triku.obs.n_counts / np.median(adata_triku.obs.n_counts)
    ...
    pd.DataFrame({"cluster_labels": cluster_labels}, index=list(adata.obs_names)).to_csv(
        "output/" + args.name + "_predsc100.50.csv")
    # index即行标签
