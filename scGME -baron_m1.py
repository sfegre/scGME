...
class SCClust():
    def __init__(self, dip_merge_threshold=0.9, batch_size=256,learning_rate=1e-4, pretrain_epochs=50, embedding_size=47, dedc_epochs=10, lambda_1=2.5, lambda_2=1.5, n_clusters_max=np.inf, n_clusters_min=5):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', type=str, default='baron_m1')
    parser.add_argument('--data_type', default='txt')
    ...
    data = pd.read_table("data/baron_m1/data.txt", sep='\t',engine='python')
    y = pd.read_table("data/baron_m1/label.txt", sep='\t', engine='python',index_col=0,header=None,names=['cell','labels'])
    ...
    print(data)
    y_class = {'alpha': 0, 'beta': 1,'delta':2,'ductal':3,'gamma':4}
    y['labels'] = y['labels'].map(y_class)
